...
def get_email
  ...
  if ENV["ADMIN_EMAIL"].present?
    puts "Loaded admin email from environment variables: #{ENV["ADMIN_EMAIL"]}"
    ENV["ADMIN_EMAIL"]
  elsif Rails.application.credentials.admin_email.present?
    puts "Loaded admin email from config/credentials.yml.enc: #{Rails.application.credentials.admin_email}"
...
def get_password
  ...
  if ENV["ADMIN_PASSWORD"].present?
    puts "Loaded admin password from environment variables: #{ENV["ADMIN_PASSWORD"]}"
    ENV["ADMIN_PASSWORD"]
  elsif Rails.application.credentials.admin_password.present?
    puts "Loaded admin password from config/credentials.yml.enc: #{Rails.application.credentials.admin_password}"
